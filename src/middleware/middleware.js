...
const authentication = async function (req, res, next) {
  try {
    ...
    if (!token) { return res.status(404).send({ status: false, message: "Please pass token" });
    }
    ...
    try {
      ...
      jwt.verify(token, secretkey, async function (error, decode) {
        ...
        let userPresent = await userModel.findOne({_id: userId});
          ...
          if (!userPresent) {
            return res.status(404).send({ status: false, msg: `user with this ID: ${userId} is not found` });
          }
